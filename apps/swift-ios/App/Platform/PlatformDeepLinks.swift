...
enum PlatformRoute: Codable, Hashable, Identifiable, Sendable {
    #if DEBUG
    static let nativeScheme = "cocoa-dev"
    #else
    static let nativeScheme = "cocoa"
    #endif
...
enum PlatformDeepLinkError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedURL:
            "That Cocoa link is not supported."
        case .missingIdentifier:
            "That Cocoa link is missing its destination."
        case .invalidIdentifier:
            "That Cocoa link contains an invalid destination."
        }
    }
}
