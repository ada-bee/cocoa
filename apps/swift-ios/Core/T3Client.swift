...
public actor EnvironmentRuntime {
    ...
    /// Cocoa clients own one gateway connection. This migration keeps the
    /// active gateway (or the first saved gateway) and removes stale upstream
    /// multi-environment credentials and clients.
    public func enforceSingleGateway() async throws {
        ...
        let retained = environments.first(where: { $0.id == activeID }) ?? environments[0]
        for environment in environments where environment.id != retained.id {
            try await remove(id: environment.id)
        }
        try await environmentStore.setActiveEnvironment(id: retained.id)
    }
    ...
    public func pair(url: String, clientLabel: String? = nil) async throws -> T3Client {
        ...
        try await removeAllGateways(except: environment.id)
        return await client(for: environment)
    }
    ...
    private func removeAllGateways(except retainedID: String) async throws {
        let environments = try await environmentStore.load()
        for environment in environments where environment.id != retainedID {
            try await remove(id: environment.id)
        }
        try await environmentStore.setActiveEnvironment(id: retainedID)
    }
