...
public enum PairingURLError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidQRCode: "Scanned QR code is not a Cocoa pairing link."
        }
    }
}
...
public enum PairingURL {
    ...
    /// QR scanner. Cocoa and legacy T3 pairing wrappers are unwrapped.
    ...
    private static func extractPairingURL(
        ...
        guard let components = URLComponents(string: trimmed),
              ["cocoa", "cocoa-dev", "t3code", "t3code-swiftui", "t3code-swiftui-dev"]
              .contains(components.scheme?.lowercased() ?? "")
        else {
            return trimmed
        }
