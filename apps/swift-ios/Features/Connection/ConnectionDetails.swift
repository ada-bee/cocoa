...
enum ConnectionDetailsError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .empty:
            "Paste a Cocoa pairing link or enter your gateway address."
        case .invalidAddress:
            "That connection link does not include a valid gateway address."
        case .unsupportedScheme:
            "Use an HTTP or HTTPS Cocoa gateway address."
        }
    }
}
...
enum ConnectionDetailsParser {
    ...
    static func parse(_ input: String) throws -> ConnectionDetails {
        ...
        let extracted = ["cocoa:", "cocoa-dev:", "t3code:"].contains {
        ...
        if candidate.contains("://")
            || loweredCandidate.hasPrefix("cocoa:")
            || loweredCandidate.hasPrefix("cocoa-dev:")
    ...
    private static func parseURL(_ input: String) throws -> ConnectionDetails {
        ...
        if ["cocoa", "cocoa-dev", "t3", "t3code", "t3code-swiftui", "t3code-swiftui-dev"].contains(scheme) {
    ...
    private static func firstURL(in input: String) -> String? {
        guard let expression = try? NSRegularExpression(
            pattern: #"(?i)(?:(?:https?|wss?|t3)://|(?:cocoa(?:-dev)?|t3code(?:-swiftui(?:-dev)?)?):(?://)?)[^\s<>"']+"#
        ) else {
            return nil
        }
...
enum ConnectionErrorCopy {
    static func message(for rawMessage: String?) -> String {
        ...
        if message.isEmpty || message == "cancelled" || message == "canceled" {
            return "The connection stopped before it finished. Make sure the Cocoa gateway is running, then try again."
        }
        ...
        if message.contains("timed out") || message.contains("timeout") {
            return "The gateway took too long to respond. Check the address and network route."
        }
        if message.contains("offline") || message.contains("network")
            || message.contains("could not connect") || message.contains("not connected") {
            return "This iPhone could not reach the gateway. Check the address and network, then try again."
        }
        return "Cocoa could not complete pairing. Check the gateway address and use a fresh pairing code."
    }
}
