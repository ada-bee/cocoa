...
public struct SettingsView: View {
    ...
    private var connectionSection: some View {
        SettingsSection(title: "Cocoa gateway") {
            VStack(spacing: 0) {
                if model.snapshot.environments.isEmpty {
                    connectionFallbackRow
                } else if let gateway = model.snapshot.environments.first {
                    environmentRow(gateway)
                }
                ...
                Button {
                    ...
                    SettingsActionRow(
                        title: model.snapshot.environments.isEmpty
                            ? "Pair gateway"
                            : "Replace gateway",
                        systemImage: "plus"
                    )
    ...
    private var aboutSection: some View {
        SettingsSection(title: "About") {
            VStack(spacing: 0) {
                ...
                settingsDivider
                Link(destination: URL(string: "https://github.com/ada-bee/cocoa")!) {
                    SettingsNavigationRow(
                        title: "Open source",
    ...
    private var connectionFallbackRow: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(model.snapshot.connection.environmentName ?? "Cocoa gateway")
    ...
    private func environmentRow(_ environment: FeatureEnvironment) -> some View {
        HStack(spacing: 12) {
            ...
            let isConnected = model.snapshot.connection.state == .connected
            SettingsRowIcon(
                systemName: isConnected ? "checkmark.circle.fill" : "network.slash",
                color: isConnected ? T3Colors.success : T3Colors.textTertiary
            )
        ...
        .frame(minHeight: 62)
        .accessibilityElement(children: .combine)
        .accessibilityHint("Current Cocoa gateway")
    }
    ...
    private var connectionDescription: String {
        model.snapshot.connection.endpoint ?? "No paired gateway"
    }
