...
struct PlatformDeepLinkTests {
    ...
    func parsesWidgetThreadRoute() throws {
        let route = try PlatformDeepLinkParser.parse(
            "cocoa://threads/environment-1/thread-7"
        )
    ...
    func parsesProjectAndEnvironmentQueryRoutes() throws {
        #expect(
            try PlatformDeepLinkParser.parse("cocoa://projects/project-7?environment=environment-1")
                == .project(environmentID: "environment-1", projectID: "project-7")
        )
        #expect(
            try PlatformDeepLinkParser.parse("cocoa://environments/environment-2")
                == .environment(id: "environment-2")
        )
        #expect(
            try PlatformDeepLinkParser.parse("cocoa://new-task?environment=environment-2&project=project-8")
    ...
    func unwrapsPairingURL() throws {
        let route = try PlatformDeepLinkParser.parse(
            "cocoa://pair?pairingUrl=https%3A%2F%2Fremote.example.com%2Fpair%23token%3DPAIR"
        )
    ...
    @Test
    func rejectsWebNavigationRoutes() {
        #expect(throws: PlatformDeepLinkError.unsupportedURL) {
            try PlatformDeepLinkParser.parse("https://app.t3.codes/threads/env/thread")
        }
    }
    ...
    @Test
    func acceptsCocoaIdentitiesAndLegacyPairingRoutes() throws {
        #expect(
            try PlatformDeepLinkParser.parse("cocoa://threads/environment/thread")
                == .thread(environmentID: "environment", threadID: "thread")
        )
        #expect(
            try PlatformDeepLinkParser.parse("cocoa-dev://threads/environment/thread")
                == .thread(environmentID: "environment", threadID: "thread")
        )
        #expect(
