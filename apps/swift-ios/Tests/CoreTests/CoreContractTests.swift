...
final class CoreContractTests: XCTestCase {
    ...
    func testRuntimeMigrationRetainsOnlyTheActiveGatewayAndCredential() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("cocoa-single-gateway-\(UUID().uuidString)", isDirectory: true)
        let file = directory.appendingPathComponent("environments.json")
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        let active = Environment(
            ...
            httpBaseURL: URL(string: "https://active.example")!,
            webSocketBaseURL: URL(string: "wss://active.example")!
        )
        let store = EnvironmentStore(fileURL: file)
        try await store.save([first, active])
        try await store.setActiveEnvironment(id: active.id)
        let credentials = InMemoryCredentialStore(credentials: [
            first.id: EnvironmentCredential(accessToken: "old-token"),
            active.id: EnvironmentCredential(accessToken: "active-token"),
        ])
        let runtime = EnvironmentRuntime(
            environmentStore: store,
        ...
        try await runtime.enforceSingleGateway()

        let remainingIDs = try await runtime.environments().map(\.id)
        let activeID = try await store.activeEnvironmentID()
        let removedCredential = await credentials.credential(for: first.id)
        let retainedCredential = await credentials.credential(for: active.id)
        XCTAssertEqual(remainingIDs, [active.id])
        XCTAssertEqual(activeID, active.id)
        XCTAssertNil(removedCredential)
        XCTAssertEqual(retainedCredential?.accessToken, "active-token")
    }
