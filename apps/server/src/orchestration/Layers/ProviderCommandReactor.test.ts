...
describe("ProviderCommandReactor", () => {
  ...
  it("generates a thread title on the first turn", async () => {
    const ownedModelSelection = createModelSelection(
  ...
  it("does not generate or locally rename a provider-owned worktree branch", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("rejects a first turn on a provider instance different from the owning project", async () => {
    const harness = await createHarness({
    ...
    expect(failure.message).toContain(
      "Model selection provider instance 'claudeAgent' must match owning project 'project-1' provider instance 'codex'",
    );
    expect(harness.startSession).not.toHaveBeenCalled();
    expect(harness.sendTurn).not.toHaveBeenCalled();
  });
  ...
  it("rejects switching an existing Codex thread to another Codex instance", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    expect(failure.message).toContain(
      "Model selection provider instance 'codex_work' must match owning project 'project-1' provider instance 'codex'",
    );
  ...
  it("rejects provider changes at the project boundary after a session is bound", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("rejects cross-driver provider changes before restarting a stopped session", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
