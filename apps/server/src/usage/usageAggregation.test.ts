...
function aggregate(records: readonly UsageRecord[], timeZone = "UTC") {
  ...
  for (const item of records) {
    aggregator.add(item, { hostId: "host-a", sourceId: `${item.provider}-source` });
  }
...
describe("UsageAggregator", () => {
  ...
  it("keeps matching provider IDs from different physical sources", () => {
    const aggregator = new UsageAggregator({
      timeZone: "UTC",
      sinceDay: "2026-08-01",
      untilDay: "2026-08-31",
    ...
    const item = record({ dedupeKey: "msg_shared:" });
    expect(aggregator.add(item, { hostId: "host-a", sourceId: "source-a" })).toBe(true);
    expect(aggregator.add(item, { hostId: "host-b", sourceId: "source-b" })).toBe(true);
    expect(aggregator.finish().duplicatesDropped).toBe(0);
  });
  ...
  it("reports whether a record contributed", () => {
    ...
    const source = { hostId: "host-a", sourceId: "claude-source" };
    expect(aggregator.add(record({ dedupeKey: "msg_1:" }), source)).toBe(true);
    expect(aggregator.add(record({ dedupeKey: "msg_1:" }), source)).toBe(false);
    expect(
      aggregator.add(record({ timestampMs: Date.parse("2026-07-01T12:00:00Z") }), source),
    ).toBe(false);
  });
  ...
  it("keeps identical provider/model cells separate by physical source", () => {
    const aggregator = new UsageAggregator({
      timeZone: "UTC",
      sinceDay: "2026-08-01",
      untilDay: "2026-08-31",
    ...
    aggregator.add(record(), { hostId: "host-a", sourceId: "source-a" });
    aggregator.add(record(), { hostId: "host-b", sourceId: "source-b" });

    const result = aggregator.finish();

    expect(result.buckets).toHaveLength(2);
    expect(result.buckets.map((bucket) => bucket.source.sourceId)).toEqual([
      "source-a",
      "source-b",
    ]);
  });
