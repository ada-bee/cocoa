...
export interface CodexAdapterCoreOptions {
  ...
  /** Legacy adapters may supply their process cwd; endpoints must require an explicit remote cwd. */
  ...
  readonly closeNativeEventLoggerOnRelease?: boolean;
 ...
 */
export const makeCodexAdapterCore = Effect.fn("makeCodexAdapterCore")(function* (
  codexConfig: CodexSettings,
  options: CodexAdapterCoreOptions,
) {
  ...
  const managedNativeEventLogger = options.closeNativeEventLoggerOnRelease
    ? nativeEventLogger
    : undefined;
      ...
      Effect.gen(function* () {
        ...
        if (cwd === undefined) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            issue: `Provider instance '${boundInstanceId}' requires an explicit remote workspace cwd.`,
          });
        }
        const runtimeInput: CodexEndpointSessionRuntimeOptions = {
        ...
        const runtime = yield* options.makeRuntime(runtimeInput).pipe(
