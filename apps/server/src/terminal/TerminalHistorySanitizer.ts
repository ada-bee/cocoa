/**
 * Removes terminal request/reply traffic that is unsafe to replay while
 * preserving control sequences that contribute visible terminal state.
 *
 * A control sequence can be split across output chunks, so callers retain the
 * returned pending sequence and pass it to the next invocation. Pending data
 * should be discarded when the terminal stream ends or its history is reset.
 */

export interface TerminalHistorySanitizationResult {
  readonly visibleText: string;
  readonly pendingControlSequence: string;
}
...
function shouldStripCsiSequence(body: string, finalByte: string): boolean {
  ...
  // DECRQM mode queries (...$p) and DECRPM replies (...$y) are request/reply
  // traffic. The `$` guard preserves setters such as DECSTR (!p) and DECSCL
  // ("p), which have replayable terminal-state value.
  if ((finalByte === "p" || finalByte === "y") && /^[0-9;?]*\$$/.test(body)) {
    return true;
  }
  // XTVERSION query (>q). DECSCUSR (space-intermediate q) remains replayable.
  if (finalByte === "q" && /^>[0-9;]*$/.test(body)) {
    return true;
  }
  // Kitty keyboard protocol query/reply (?u). Restore-cursor (bare u) remains.
...
// DECRQSS ($q) and XTGETTCAP (+q) queries plus their replies ([01]$r /
// [01]+r) are pure request/response traffic. Replaying a query causes the
// terminal to emit a fresh reply into the shell prompt.
...
export function sanitizeTerminalHistoryChunk(
  ...
  data: string,
): TerminalHistorySanitizationResult {
