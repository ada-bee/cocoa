...
export function hasIncompleteUsageSources(
  environments: readonly Pick<EnvironmentUsage, "summary">[],
): boolean {
  return environments.some((environment) =>
    environment.summary.sources.some((source) => source.status !== "ok"),
  );
}

/** True when every answered gateway has no complete or partial transcript source. */
export function hasNoReportedUsageCoverage(
  environments: readonly Pick<EnvironmentUsage, "summary">[],
): boolean {
  return (
    ...
    environments.every((environment) =>
      environment.summary.sources.length > 0
        ? environment.summary.sources.every((source) => !sourceCanContribute(source.status))
        : environment.summary.coverage !== undefined &&
          environment.summary.coverage.hosts.every((host) => host.status !== "reported"),
    )
  );
}
...
/**
 * Two sources are the same physical transcript directory only when their
 * host-generated opaque identity agrees.
 *
 * `sourceId` is host-generated from the installation and local directory
 * identity, so matching display labels on different machines do not collide.
 */
function fingerprintKey(fingerprint: UsageSourceFingerprint): string {
  return [fingerprint.hostId, fingerprint.provider, fingerprint.sourceId].join(" ");
}

const sourceCanContribute = (status: UsageSummary["sources"][number]["status"]): boolean =>
  status === "ok" || status === "partial";

const sourceQuality = (status: UsageSummary["sources"][number]["status"]): number =>
  status === "ok" ? 2 : status === "partial" ? 1 : 0;
...
function claimSources(environments: readonly EnvironmentUsage[]): {
  ...
  const winnerByFingerprint = new Map<
    string,
    {
      readonly environmentId: EnvironmentId;
  ...
  for (const environment of ordered) {
    for (const source of environment.summary.sources) {
      if (!sourceCanContribute(source.status)) continue;
      const key = fingerprintKey(source.fingerprint);
      const quality = sourceQuality(source.status);
      candidates.push({
        key,
        environmentId: environment.environmentId,
        environmentLabel: environment.label,
        sourceLabel: source.fingerprint.label,
      });
      const winner = winnerByFingerprint.get(key);
      if (winner === undefined || quality > winner.quality) {
        winnerByFingerprint.set(key, { environmentId: environment.environmentId, quality });
      }
    }
  }

  const ownerByFingerprint = new Map(
    [...winnerByFingerprint].map(([key, winner]) => [key, winner.environmentId] as const),
  );
  const duplicates = candidates.flatMap((candidate) =>
    ownerByFingerprint.get(candidate.key) === candidate.environmentId
      ? []
      : [`${candidate.environmentLabel}: ${candidate.sourceLabel}`],
  );
...
function ownedContribution(
  ...
  return {
    buckets: environment.summary.buckets.filter((bucket) =>
      ownedSources.has([bucket.source.hostId, bucket.provider, bucket.source.sourceId].join(" ")),
    ),
    sessions,
  };
}
