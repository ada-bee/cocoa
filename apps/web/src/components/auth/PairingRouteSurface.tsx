  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">Sign in to Cocoa</h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
        ...
        <form className="mt-6 space-y-4" onSubmit={(event) => void handleSubmit(event)}>
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="pairing-token">
              Cocoa password or pairing token
            </label>
            <Input
              id="pairing-token"
              autoCapitalize="none"
              ...
              placeholder="Enter your Cocoa password"
              type="password"
              spellCheck={false}
              value={credential}
            />
...
function describeAuthGate(bootstrapMethods: ReadonlyArray<string>): string {
  ...
  return "Enter the Cocoa password to create a browser session, or open a one-time pairing link.";
}
...
function describeSupportedMethods(bootstrapMethods: ReadonlyArray<string>): string {
  ...
  return "The Cocoa password is exchanged once for this browser session. One-time pairing links are also accepted.";
}
