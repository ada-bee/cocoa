...
export function UsagePage() {
  ...
  const { merged, environments, isPending, isPartial, isUnavailable, refresh } = useUsage(window);
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-6">
        ...
        {isPending ? (
          ...
          <div className="border border-border px-4 py-12 text-center">
            <h2 className="text-sm font-medium text-foreground">Usage reporting unavailable</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              No connected provider host could report transcript usage for this window.
            </p>
          </div>
        ) : (
          <>
...
export function collectIncompleteUsageSources(
  environments: readonly {
    ...
    summary?: {
      sources: readonly {
        ...
        message: string | null;
        fingerprint: { hostId: string; sourceId: string; label: string };
      }[];
    } | null;
  }[],
) {
  return environments.flatMap((environment) =>
    (environment.summary?.sources ?? []).flatMap((source) =>
      source.status === "ok" ? [] : [{ environmentLabel: environment.label, ...source }],
    ),
  );
}
...
function UsageCoverageNotice({
  ...
  readonly environments: readonly {
    ...
    summary?: {
      ...
      coverage?: {
        state: "complete" | "partial" | "allUnsupported";
        hosts: readonly {
          providerHostId: string;
          status: "reported" | "unsupported" | "failed" | "incompatible";
          message: string | null;
        }[];
      };
    } | null;
  ...
  const incompleteHosts = environments.flatMap((environment) =>
    (environment.summary?.coverage?.hosts ?? []).flatMap((host) =>
      host.status === "reported" ? [] : [{ environmentLabel: environment.label, ...host }],
    ),
  );
  const incompleteSources = collectIncompleteUsageSources(environments);
  const hasUnknownCoverage = environments.some(
    (environment) => environment.summary !== null && environment.summary?.coverage === undefined,
  );
  if (
    failed.length === 0 &&
    stale.length === 0 &&
  ...
  return (
    <div className="flex flex-col gap-1 border border-border px-3 py-2 text-xs text-muted-foreground">
      {isPartial ? <span>Some usage sources are incomplete. Totals are partial.</span> : null}
      {hasUnknownCoverage ? (
        <span>Some environments did not report host coverage. Totals may be incomplete.</span>
      ) : null}
      ...
      {incompleteHosts.map((host) => (
        <span key={`${host.environmentLabel}:${host.providerHostId}`}>
          {host.environmentLabel} host {host.providerHostId}: {host.message ?? host.status} Totals
          are incomplete.
        </span>
      ))}
      {incompleteSources.map((source) => (
        <span
          key={`${source.environmentLabel}:${source.fingerprint.hostId}:${source.fingerprint.sourceId}`}
        >
          {source.environmentLabel} source {source.fingerprint.label}:{" "}
          {source.message ?? source.status}. Totals are incomplete.
        </span>
      ))}
      {duplicateSources.length > 0 ? (
        <span>
