import { DesktopUpdateChannelSchema, type DesktopUpdateChannel } from "@t3tools/contracts/ipc";
...
function normalizeDesktopSettingsDocument(
  ...
  return {
    ...
    updateChannelConfiguredByUser: configured,
  };
}
...
function setUpdateChannel(
  ...
  return settings.updateChannel === channel && settings.updateChannelConfiguredByUser
    ? settings
    : { ...settings, updateChannel: channel, updateChannelConfiguredByUser: true };
}
...
const writeSettings = Effect.fn("desktop.settings.writeSettings")(function* (input: {
  ...
  const temporaryPath = `${input.settingsPath}.${process.pid}.${input.suffix}.tmp`;
