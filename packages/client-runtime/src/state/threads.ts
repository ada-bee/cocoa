...
interface ThreadCacheMeta {
  readonly cacheEpoch: string;
  readonly cacheRevision: number;
}

function cacheMetaFromSnapshot(
  snapshot: OrchestrationThreadDetailSnapshot,
): Option.Option<ThreadCacheMeta> {
  const cacheEpoch = snapshot.cacheEpoch ?? snapshot.page?.cacheEpoch;
  const cacheRevision = snapshot.cacheRevision ?? snapshot.page?.cacheRevision;
  return cacheEpoch === undefined || cacheRevision === undefined
    ? Option.none()
    : Option.some({ cacheEpoch, cacheRevision });
}

function cacheMetaFields(meta: Option.Option<ThreadCacheMeta>) {
  return Option.match(meta, {
    onNone: () => ({}),
    onSome: ({ cacheEpoch, cacheRevision }) => ({ cacheEpoch, cacheRevision }),
  });
}
...
export const makeEnvironmentThreadState = Effect.fn("EnvironmentThreadState.make")(function* (
  ...
  // Cocoa's provider cache generation/revision is retained for diagnostics and
  // persistence. It is deliberately not a page-consistency token: unrelated
  // provider sweeps advance the global revision.
  const loadedCacheMeta = yield* Ref.make(Option.flatMap(cached, cacheMetaFromSnapshot));
  // Unlike the global cache revision, this changes only when this provider
  // thread's authoritative history changes, so unrelated sweeps cannot make
  // every subsequent older-page request stale.
  const loadedHistoryVersion = yield* Ref.make(
    Option.flatMap(cached, (snapshot) => Option.fromUndefinedOr(snapshot.page?.historyVersion)),
  );
  ...
  const applyItemLocked = Effect.fn("EnvironmentThreadState.applyItemLocked")(function* (
    ...
    if (item.kind === "snapshot") {
      ...
      yield* Ref.set(loadedCacheMeta, cacheMetaFromSnapshot(item.snapshot));
      yield* Ref.set(
        loadedHistoryVersion,
        Option.fromUndefinedOr(item.snapshot.page?.historyVersion),
      );
  ...
  const loadOlderTurns = Effect.fn("EnvironmentThreadState.loadOlderTurns")(function* () {
    ...
    let mayRefreshCurrentWindow = true;
    ...
    while (true) {
      ...
      const window: ThreadSnapshotWindow = {
        ...
        beforeCursor: requestedPage.beforeCursor!,
      };
      ...
      const outcome = yield* applyLock.withPermits(1)(
        Effect.gen(function* () {
          const epochNow = yield* Ref.get(historyEpoch);
          const loadedSequence = yield* SubscriptionRef.get(lastSequence);
          const currentHistoryVersion = yield* Ref.get(loadedHistoryVersion);
          const responseHistoryVersion = Option.flatMap(response, (snapshot) =>
            Option.fromUndefinedOr(snapshot.page?.historyVersion),
          );
          const historyVersionMismatch =
            Option.isSome(currentHistoryVersion) || Option.isSome(responseHistoryVersion)
              ? Option.isNone(currentHistoryVersion) ||
                Option.isNone(responseHistoryVersion) ||
                currentHistoryVersion.value !== responseHistoryVersion.value
              : false;
          ...
          const staleSequence = Option.match(response, {
            onNone: () => false,
          ...
          if (
            ...
            mayRefreshCurrentWindow
          ) {
            return "refresh-current-window" as const;
          }
          if (
            Option.isNone(response) ||
            epochNow !== epochAtStart ||
            historyVersionMismatch ||
            (Option.isNone(currentHistoryVersion) && staleSequence)
          ) {
            yield* SubscriptionRef.update(state, (value) => ({
              ...value,
              page: Option.map(value.page, (existing) => ({ ...existing, loadingOlder: false })),
      ...
      // The thread itself advanced after the visible window was loaded (for
      // example, its active turn completed). Refresh that current window once,
      // adopt its new cursor/version, and retry automatically. Bounding this
      // to one refresh prevents a rapidly changing thread from spinning while
      // still avoiding a permanent "Load earlier" dead end.
      mayRefreshCurrentWindow = false;
      const refreshEpochAtStart = yield* Ref.get(historyEpoch);
      const refreshState = yield* SubscriptionRef.get(state);
      const loadedUserTurnCount = Option.match(refreshState.data, {
        onNone: () => 0,
        onSome: (thread) =>
          new Set(
            thread.messages
      ...
      const refreshed = yield* snapshotLoader.load(prepared, threadId, {
        // Preserve the already widened window instead of collapsing a user
        // who loaded 30/50 turns back to the initial ten on recovery.
        turnLimit: Math.max(INITIAL_THREAD_USER_TURN_LIMIT, loadedUserTurnCount),
      });
      const retryPage = yield* applyLock.withPermits(1)(
        Effect.gen(function* () {
          const epochNow = yield* Ref.get(historyEpoch);
          const loadedSequence = yield* SubscriptionRef.get(lastSequence);
          if (
            Option.isNone(refreshed) ||
            epochNow !== refreshEpochAtStart ||
            refreshed.value.snapshotSequence < loadedSequence
          ) {
            yield* SubscriptionRef.update(state, (value) => ({
              ...value,
              page: Option.map(value.page, (existing) => ({ ...existing, loadingOlder: false })),
          ...
          yield* Ref.set(loadedCacheMeta, cacheMetaFromSnapshot(refreshed.value));
          yield* Ref.set(
            loadedHistoryVersion,
            Option.fromUndefinedOr(refreshed.value.page?.historyVersion),
          );
          const refreshedPage = pageStateFromSnapshot(refreshed.value.page);
          yield* setThread(refreshed.value.thread, refreshedPage);
          const retry = Option.getOrNull(refreshedPage);
          if (retry === null || !retry.hasMore || retry.beforeCursor === null) {
            return null;
          }
          yield* SubscriptionRef.update(state, (value) => ({
            ...value,
  ...
  yield* Effect.addFinalizer(() =>
    Effect.all([
      ...
      Ref.get(loadedCacheMeta),
    ]).pipe(
      Effect.flatMap(([current, snapshotSequence, cacheMeta]) =>
