...
function userMessage(id: string, turnId: string, createdAt: string): OrchestrationMessage {
  return { ...message(id, turnId, createdAt), role: "user" };
}
...
const CACHE_EPOCH = "provider-cache-epoch";
const CACHE_WINDOWED_SNAPSHOT: OrchestrationThreadDetailSnapshot = {
  ...WINDOWED_SNAPSHOT,
  cacheEpoch: CACHE_EPOCH,
  cacheRevision: 7,
  page: {
    ...WINDOWED_SNAPSHOT.page!,
    cacheEpoch: CACHE_EPOCH,
    cacheRevision: 7,
    historyVersion: "history-v1",
  },
};

const CACHE_OLDER_PAGE: OrchestrationThreadDetailSnapshot = {
  ...OLDER_PAGE,
  cacheEpoch: CACHE_EPOCH,
  cacheRevision: 7,
  page: {
...
const makeHarness = Effect.fn("TestThreadPagination.makeHarness")(function* (options?: {
  ...
  /** Response for a bounded current-window refresh after a history-version mismatch. */
  readonly refreshResponse?: LoaderResponse;
  ...
  const currentWindowLoads = yield* Ref.make(0);
      ...
      Ref.update(loaderWindows, (current) => [...current, window]).pipe(
        Effect.andThen(
          window?.beforeCursor === undefined
            ? Ref.getAndUpdate(currentWindowLoads, (count) => count + 1).pipe(
                Effect.map((count) =>
                  count === 0
...
describe("thread pagination state", () => {
  ...
  it.effect("uses thread history version instead of global cache or orchestration revisions", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        initialResponse: Option.some(CACHE_WINDOWED_SNAPSHOT),
      });
      yield* harness.awaitState((value) => Option.isSome(value.page));
      ...
      yield* harness.resolveNextPage(
        Option.some({
          ...
          snapshotSequence: 5,
          cacheRevision: 8,
          page: { ...CACHE_OLDER_PAGE.page!, cacheRevision: 8 },
        }),
      );
  ...
  it.effect("rejects an older page from a different thread history version", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        initialResponse: Option.some(CACHE_WINDOWED_SNAPSHOT),
        refreshResponse: Option.none(),
      });
      yield* harness.awaitState((value) => Option.isSome(value.page));
      ...
      yield* harness.resolveNextPage(
        Option.some({
          ...CACHE_OLDER_PAGE,
          page: { ...CACHE_OLDER_PAGE.page!, historyVersion: "history-v2" },
        }),
      );
  ...
  it.effect("refreshes and retries when the open thread history version advances", () =>
    Effect.gen(function* () {
      const widenedMessages = Array.from({ length: 30 }, (_, index) =>
        userMessage(
      ...
      const widenedWindow: OrchestrationThreadDetailSnapshot = {
        ...CACHE_WINDOWED_SNAPSHOT,
        thread: { ...BASE_THREAD, messages: [...widenedMessages, RECENT_MESSAGE] },
      };
      const refreshedWindow: OrchestrationThreadDetailSnapshot = {
        ...widenedWindow,
        snapshotSequence: 11,
        cacheRevision: 8,
        thread: {
          ...widenedWindow.thread,
          title: "Turn completed",
        },
        page: {
          ...CACHE_WINDOWED_SNAPSHOT.page!,
          beforeCursor: "cursor-2",
          snapshotSequence: 11,
          cacheRevision: 8,
          historyVersion: "history-v2",
        },
      };
      const harness = yield* makeHarness({
        initialResponse: Option.some(widenedWindow),
        refreshResponse: Option.some(refreshedWindow),
      });
      yield* harness.awaitState((value) => Option.isSome(value.page));
      ...
      // The first page races a normal same-thread update. The state machine
      // must refresh its current window and issue a second older-page request
      // using the refreshed cursor without requiring another UI click/remount.
      yield* harness.resolveNextPage(
        Option.some({
          ...CACHE_OLDER_PAGE,
          cacheRevision: 8,
          page: {
            ...CACHE_OLDER_PAGE.page!,
            cacheRevision: 8,
            historyVersion: "history-v2",
          },
        }),
      ...
      const state = yield* harness.awaitState((value) => hasMessage(value, "message-old"));
      expect(Option.getOrThrow(state.data).title).toBe("Turn completed");
      expect(hasMessage(state, "message-user-1")).toBe(true);
      const windows = yield* Ref.get(harness.loaderWindows);
      expect(windows).toEqual([
        { turnLimit: INITIAL_THREAD_USER_TURN_LIMIT },
        { turnLimit: 20, beforeCursor: "cursor-1" },
        { turnLimit: 30 },
        { turnLimit: 20, beforeCursor: "cursor-2" },
      ]);
    }),
  );
