...
/**
 * Folds parsed transcript records into `(source, day, provider, model)` buckets.
 *
...
export class UsageAggregator {
   ...
   */
  add(record: UsageRecord, source: UsageBucketSource): boolean {
    if (record.dedupeKey !== null) {
      const sourceDedupeKey = `${source.hostId}\0${source.sourceId}\0${record.dedupeKey}`;
      if (this.#seen.has(sourceDedupeKey)) {
        this.#duplicatesDropped += 1;
        return false;
      }
      this.#seen.add(sourceDedupeKey);
    }
    ...
    const key = [source.hostId, source.sourceId, day, record.provider, record.model].join("\\0");
  ...
  finish(): AggregateResult {
    ...
    for (const [key, bucket] of this.#buckets) {
      const [hostId = "", sourceId = "", day = "", provider = "", model = ""] = key.split("\\0");
      buckets.push({
        source: { hostId, sourceId },
        day: day as UsageDay,
    ...
    buckets.sort(
      (a, b) =>
        ...
        a.source.hostId.localeCompare(b.source.hostId) ||
