...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Cocoa Code scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Cocoa Code terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Cocoa Code.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Cocoa Code\'s built-in icon locations.',
      },
